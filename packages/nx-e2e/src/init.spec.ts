...
describe.each(WORKSPACE_TYPES)(
  '@tactical-ddd/nx init generator (e2e) (%s)',
  (workspaceType) => {
    ...
    beforeAll(() => {
      projectDirectory = createTestProject('test-project-init', workspaceType);
    ...
    describe('dependency installation', () => {
      it('adds the generator plugin packages to the workspace package.json', () => {
        const devDependencies =
          ws.readJson('package.json').devDependencies ?? {};
    ...
    describe('shared kernel scaffolding', () => {
      it.each(LAYERS)(
        'scaffolds the "%s" library under libs/shared',
        (layer) => {
